...
static yaw_para_change_s yaw_data;
...
{
    # define MID_STATE 3
    static int prev_switch_state = -1;  // 初始化为非中档值
    ...
    // 离开中档时重置状态
    if (prev_switch_state == MID_STATE) {
        prev_switch_state = -1;  // 标记为非中档状态
    }
    }
    else if (switch_is_mid(rc_data[TEMP].rc.switch_right)) // 右侧开关状态[中],底盘和云台分离,底盘保持不转动
    {

        // // 静态变量记录上一次状态（初始值设为非中档）
    // 仅在首次进入中档时执行退弹
    if (prev_switch_state != MID_STATE) {
        shoot_cmd_send.load_mode = LOAD_Withdraw_the_bullet;  // 单次触发
        prev_switch_state = MID_STATE;  // 更新状态
    } else {
        shoot_cmd_send.load_mode = LOAD_STOP;  // 后续保持停止
    }
        ...
        shoot_cmd_send.friction_mode = FRICTION_OFF;
        // shoot_cmd_send.load_mode=LOAD_STOP;
         shoot_cmd_send.load_mode=LOAD_Withdraw_the_bullet;
        gimbal_cmd_send.lid_mode=LID_CLOSE;
...
vision_recv_data; // 视觉接收数据指针,初始化时返回
...
static void RemoteShootSet()
{
        ...
        shoot_cmd_send.load_mode=LOAD_STOP;
        gimbal_cmd_send.lid_mode=LID_CLOSE;
    if (switch_is_up(rc_data[TEMP].rc.switch_right)) //右侧开关状态[上]（还是视觉模式）
    {
        // yaw_data.yaw_para = yaw_para_1;
        // chassis_cmd_send.chassis_mode = CHASSIS_FOLLOW_GIMBAL_YAW;
        // gimbal_cmd_send.gimbal_mode = GIMBAL_GYRO_MODE;
        // chassis_cmd_send.chassis_mode = CHASSIS_ZERO_FORCE;
        // gimbal_cmd_send.gimbal_mode = GIMBAL_FREE_MODE;
        ...
        chassis_cmd_send.chassis_mode = CHASSIS_FOLLOW_GIMBAL_YAW;
        gimbal_cmd_send.gimbal_mode = GIMBAL_GYRO_MODE;
        shoot_cmd_send.friction_mode = FRICTION_OFF;//关闭
    ...
    if (switch_is_mid(rc_data[TEMP].rc.switch_right)) //右侧开关状态[中]，开启摩擦轮（开启摩擦轮，单发）
    {
//原
        // chassis_cmd_send.chassis_mode = CHASSIS_ZERO_FORCE;
        // gimbal_cmd_send.gimbal_mode = GIMBAL_FREE_MODE;
...
//
        ...
        // shoot_cmd_send.friction_mode = FRICTION_OFF;
        // shoot_cmd_send.friction_mode = FRICTION_OFF;
        // shoot_cmd_send.load_mode=LOAD_STOP;//单发
        shoot_cmd_send.load_mode=LOAD_STOP;//停止发射
    ...
    if (switch_is_down(rc_data[TEMP].rc.switch_right)) // 右侧开关状态[下]，发射弹
    {
        ...
        // &&(vision_recv_data->pitch-gimba_IMU_data_self->Pitch)>-0.5&&(vision_recv_data->pitch-gimba_IMU_data_self->Pitch)<0.5
    ...
    chassis_cmd_send.wz = 600.0f;   //600
    shoot_cmd_send.shoot_rate = 1;
        ...
        // shoot_cmd_send.shoot_mode = SHOOT_ON;  //使能shoot
        // shoot_cmd_send.load_mode=LOAD_1_BULLET; //单发
        // chassis_cmd_send.chassis_mode = CHASSIS_FOLLOW_GIMBAL_YAW;  //底盘跟随
        // gimbal_cmd_send.gimbal_mode = GIMBAL_GYRO_MODE;  //云台陀螺仪
        // gimbal_cmd_send.lid_mode=LID_CLOSE; //关闭弹舱盖



float chassis_speed_mouse=0.58;//十级10000
...
static void MouseKeySet()
{
    ...
    //默认值，防止裁判系统断连时过快
    chassis_speed_mouse=1.25;
    chassis_rotate_speed_mouse=700;
    ...
    if(chassis_power_limit>=40)
    {
        chassis_speed_mouse=1.25;
        chassis_rotate_speed_mouse=700;
        chassis_fastrotate_speed_mouse=1000;
    }
    ...
    // switch (rc_data[TEMP].key_count[KEY_PRESS][Key_R] % 3) // R键设置pid参数
    // {
    // case 0:

    //     Update_PID_Parameters(&yaw_config.controller_param_init_config,1);

    //     break;
    // case 1:

    //     Update_PID_Parameters(&yaw_config.controller_param_init_config,0);
    //     break;
    // default:
    ...
    if(rc_data[TEMP].mouse.press_l==0)//左键发射
    {
        shoot_cmd_send.load_mode = LOAD_STOP;
        chassis_cmd_send.load_mode = LOAD_STOP;
    ...
    // switch (rc_data[TEMP].key_count[KEY_PRESS][Key_S] % 2) 
    // {
    // case 0:
    // {
    //     shoot_cmd_send.load_mode = LOAD_STOP;
    ...
    // }
    // switch (rc_data[TEMP].key_count[KEY_PRESS][Key_B] % 2) // B键退弹
    // {
    // case 0:
    // {
    ...
    // }
//     static int prev_parity = 0; // 静态变量记录前一次的奇偶性
// //测试退弹
//     int current_parity = rc_data[TEMP].key_count[KEY_PRESS][Key_B] % 2;
    
//     switch (current_parity)
//     {
//     case 0:
//         shoot_cmd_send.load_mode = LOAD_STOP;
//         prev_parity = 0; // 更新为当前偶状态
//         break;
    
//     default: // current_parity 为 1（奇数次）
//         if (prev_parity == 0) 
//         {
//             // 奇偶性从偶变奇，执行一次退弹
//             // shoot_cmd_send.load_mode = LOAD_Withdraw_the_bullet;
