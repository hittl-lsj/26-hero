...
{ 
    ...
    friction_config.can_init_config.tx_id = 5;  //后级左
    friction_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_NORMAL;
    friction_lb = DJIMotorInit(&friction_config);

    friction_config.can_init_config.tx_id = 6;  //后级右
    friction_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_REVERSE;
    friction_rb = DJIMotorInit(&friction_config);

    // friction_config.can_init_config.tx_id = 7;  //前级左
    // friction_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_NORMAL;
    // friction_lf = DJIMotorInit(&friction_config);

    // friction_config.can_init_config.tx_id = 8;  //前级右
    // friction_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_REVERSE;
...
void ShootTask()
{
    ...
    if (shoot_cmd_recv.friction_mode == FRICTION_ON)
    {
        ...
        case SMALL_AMU_30:
            ...
            break;
        case BIG_AMU_16:
            // DJIMotorSetRef(friction_lf, 35000);//33000     //33000
            // DJIMotorSetRef(friction_rf, 35000);//33000    //33000
            DJIMotorSetRef(friction_lb, 48000);//46000    //60000
            DJIMotorSetRef(friction_rb, 48000);//46000   //60000
            break; 
