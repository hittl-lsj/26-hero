...
static DJIMotorInstance *motor_UP_L, *motor_UP_R; // left right forward back
static DMMotorInstance *motor_f_L, *motor_f_R; // left right forward back

static DJIMotorInstance *motor_lf, *motor_rf, *motor_lb, *motor_rb; // left right forward back
static DJIMotorInstance *chassis_motor_instance[4];
static DJIMotorInstance *chassis_moto_up_instance[2];
...
void ChassisInit()
{
    ...
    motor_UP_L=DJIMotorInit(&up_config);
    up_config.can_init_config.tx_id = 6;
    up_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_NORMAL;
    motor_UP_R=DJIMotorInit(&up_config);
    chassis_moto_up_instance[0] = motor_UP_L;
    chassis_moto_up_instance[1] = motor_UP_R;

    Motor_Init_Config_s Timing_belt_config = {
        .can_init_config = {
            .can_handle = &hcan2,
            .tx_id = 0X01,
            .rx_id = 0X04,
     ...
     motor_f_L = DMMotorInit(&Timing_belt_config);//换成达妙电机
     Timing_belt_config.can_init_config.tx_id = 0X02;
      Timing_belt_config.can_init_config.rx_id = 0X03;
      Timing_belt_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_NORMAL;
     motor_f_R = DMMotorInit(&Timing_belt_config);//换成达妙电机
...
static void OmnidirectionalCalculate()
{
    ...
    vt_lf =(-vx_dps-vy_dps)*COSECANT45 - VWHEEL_2_DPS(chassis_cmd_recv.wz * LF_CENTER);
    vt_rf =(-vx_dps+vy_dps)*COSECANT45 - VWHEEL_2_DPS(chassis_cmd_recv.wz * RF_CENTER);
    vt_lb =(vx_dps-vy_dps)*COSECANT45 - VWHEEL_2_DPS(chassis_cmd_recv.wz * LB_CENTER);
    vt_rb =(vx_dps+vy_dps)*COSECANT45 - VWHEEL_2_DPS(chassis_cmd_recv.wz * RB_CENTER);
}
...
void ChassisTask()
{
 ...
 static float sin_theta, cos_theta;//计算夹角的sin和cos值
    ...
    { // 如果出现重要模块离线或遥控器设置为急停,让电机停止
        ...
        DJIMotorStop(motor_UP_L);
        DJIMotorStop(motor_UP_R);
        DMMotorStop(motor_f_L);
        DMMotorStop(motor_f_R);
    }
    else{

        DJIMotorEnable(motor_UP_L);
        DJIMotorEnable(motor_UP_R);
        DJIMotorEnable(motor_lf);
        DJIMotorEnable(motor_rf);
        DJIMotorEnable(motor_lb);
        DJIMotorEnable(motor_rb);
        DMMotorEnable(motor_f_L);
        DMMotorEnable(motor_f_R);
    }
 ...
 switch (chassis_cmd_recv.chassis_mode)
    ...
    case CHASSIS_FOLLOW_GIMBAL_YAW: // 跟随云台,不单独设置pid,以误差角度平方为速度输出
        ...
        chassis_cmd_recv.wz = -10 * chassis_cmd_recv.offset_angle * abs(chassis_cmd_recv.offset_angle);
        break;
    ...
    DJIMotorOuterLoop(motor_UP_L, ANGLE_LOOP); 
    DJIMotorOuterLoop(motor_UP_R, ANGLE_LOOP);

    DMMotorOuterLoop(motor_f_L, SPEED_LOOP); 
    DMMotorOuterLoop(motor_f_R, SPEED_LOOP);

    switch (chassis_cmd_recv.climb_mode)
    {
    case CHASSIS_OFF_UP:
        DJIMotorSetRef(motor_UP_L,0);
        DJIMotorSetRef(motor_UP_R,0);
        break;
    case CHASSIS_CLIMB_R:
        DJIMotorSetRef(motor_UP_L,CHASSIS_MOTO_UP_INSTANCE_NUM);
        DJIMotorSetRef(motor_UP_R,0);
        break;
    case CHASSIS_CLIMB_L: 
        DJIMotorSetRef(motor_UP_L,0);
        DJIMotorSetRef(motor_UP_R,CHASSIS_MOTO_UP_INSTANCE_NUM);
        break;
        case CHASSIS_CLIMB_ALL:
        DJIMotorSetRef(motor_UP_L,CHASSIS_MOTO_UP_INSTANCE_NUM);
        DJIMotorSetRef(motor_UP_R,CHASSIS_MOTO_UP_INSTANCE_NUM);
        break;
    ...
    DMMotorSetRef(motor_f_L,10);
    DMMotorSetRef(motor_f_R,10);
