...
static DJIMotorInstance *friction_l, *friction_r,*friction_up,*loader; // 拨盘电机
...
{ 
    ...
    friction_config.can_init_config.tx_id = 4;  //后级左
    friction_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_REVERSE;
    friction_l = DJIMotorInit(&friction_config);

    friction_config.can_init_config.tx_id = 1;  //后级右
    friction_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_REVERSE;
    friction_r = DJIMotorInit(&friction_config);

    friction_config.can_init_config.tx_id = 2;  //前级左
    friction_config.controller_setting_init_config.motor_reverse_flag = MOTOR_DIRECTION_REVERSE;
...
void ShootTask()
{
    ...
    if (shoot_cmd_recv.friction_mode == FRICTION_ON)
    {
        ...
        case SMALL_AMU_15:
            ...
            DJIMotorSetRef(friction_up, 40000);
            break;
        case SMALL_AMU_18:
            // DJIMotorSetRef(friction_l, 0);
            // DJIMotorSetRef(friction_r, 0);
        ...
        case SMALL_AMU_30:
            ...
            DJIMotorSetRef(friction_up, 33000);
            break;
        case BIG_AMU_16:
            DJIMotorSetRef(friction_l, 48000);//33000     //33000
            DJIMotorSetRef(friction_r, 48000);//33000    //33000
            DJIMotorSetRef(friction_up, 48000);//46000    //60000
            break; 
        default: // 当前为了调试设定的默认值4000,因为还没有加入裁判系统无法读取弹速.
            ...
            DJIMotorSetRef(friction_up, 20000);
            break;
        }
